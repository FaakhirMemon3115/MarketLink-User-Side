...
public class DashboardController : Controller
{
    ...
    public DashboardController(IUnitOfWork unitOfWork, INotificationService notificationService)
    {
    ...
    public async Task<IActionResult> Broadcast(string targetRole, string title, string message)
    {
        ...
        var usersQuery = _unitOfWork.Repository<ApplicationUser>().Query().Where(u => u.IsActive);
    ...
    {
        var setting = await _unitOfWork.Repository<SiteSetting>().FirstOrDefaultAsync(s => s.Key == key);
        if (setting == null)
        {
            setting = new SiteSetting { Key = key, Value = value, Description = description, Group = "General" };
        ...
        else
        {
            ...
            await _unitOfWork.Repository<SiteSetting>().UpdateAsync(setting);
        }
